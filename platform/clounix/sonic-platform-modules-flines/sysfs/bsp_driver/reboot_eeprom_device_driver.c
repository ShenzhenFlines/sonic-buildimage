...
static int __init reboot_eeprom_dev_drv_init(void)
{
    ...
    REBOOTE2_INFO("reboot_eeprom_dev_drv_init...\n");
    reboot_eeprom_if_create_driver();

    ret = s3ip_sysfs_reboot_eeprom_drivers_register(&drivers);
    if (ret < 0)
    {
        REBOOTE2_ERR("reboot eeprom drivers register err, ret %d.\n", ret);
        return ret;
    }
    REBOOTE2_INFO("reboot_eeprom_dev_drv_init success.\n");
    return 0;
}
...
static void __exit reboot_eeprom_dev_drv_exit(void)
{
    ...
    REBOOTE2_INFO("reboot_eeprom_exit success.\n");
    return;
}
