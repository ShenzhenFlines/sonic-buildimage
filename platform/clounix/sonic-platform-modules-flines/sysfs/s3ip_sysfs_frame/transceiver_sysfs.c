...
static ssize_t transceiver_presence_show(struct switch_obj *obj, struct switch_attribute *attr,
                   char *buf)
{
    int ret;

    check_p(g_sff_drv);
    check_p(g_sff_drv->get_transceiver_presence_status);

    ret = g_sff_drv->get_transceiver_presence_status(buf, PAGE_SIZE);
    if (ret < 0) {
        SFF_ERR("get transceiver presence status failed, ret: %d\n", ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t transceiver_number_show(struct switch_obj *obj, struct switch_attribute *attr,
                   char *buf)
{
    check_p(g_sff_drv);
    return (ssize_t)snprintf(buf, PAGE_SIZE, "%d\n", g_sff.sff_number);
}
static ssize_t transceiver_debug_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    int ret;

    check_p(g_sff_drv);
    check_p(g_sff_drv->get_debug);

    ret = g_sff_drv->get_debug(buf, PAGE_SIZE);
    if (ret < 0) {
        SFF_ERR("get transceiver debug failed, ret: %d\n", ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t transceiver_debug_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    int ret;

    check_p(g_sff_drv);
    check_p(g_sff_drv->set_debug);
...
static ssize_t transceiver_loglevel_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    int ret;

    check_p(g_sff_drv);
    check_p(g_sff_drv->get_loglevel);

    ret = g_sff_drv->get_loglevel(buf, PAGE_SIZE);
    if (ret < 0) {
        SFF_ERR("get transceiver loglevel failed, ret: %d\n", ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t transceiver_loglevel_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    int ret;

    check_p(g_sff_drv);
    check_p(g_sff_drv->set_loglevel);
...
static struct switch_attribute transceiver_loglevel_attr = __ATTR(loglevel, S_IRUGO | S_IWUSR, transceiver_loglevel_show, transceiver_loglevel_store);

static struct attribute *transceiver_dir_attrs[] = {
    &transceiver_power_on_attr.attr,
    &transceiver_presence_attr.attr,
    &transceiver_number_attr.attr,
    &transceiver_debug_attr.attr,
    &transceiver_loglevel_attr.attr,
    NULL,
};
