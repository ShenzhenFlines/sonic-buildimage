...
static ssize_t sysled_debug_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    int ret;

    check_p(g_sysled_drv);
    check_p(g_sysled_drv->get_debug);

    ret = g_sysled_drv->get_debug(buf, PAGE_SIZE);
    if (ret < 0) {
        SYSLED_ERR("get sysled debug failed, ret: %d\n", ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t sysled_debug_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    int ret;

    check_p(g_sysled_drv);
    check_p(g_sysled_drv->set_debug);
...
static ssize_t sysled_loglevel_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    int ret;

    check_p(g_sysled_drv);
    check_p(g_sysled_drv->get_loglevel);

    ret = g_sysled_drv->get_loglevel(buf, PAGE_SIZE);
    if (ret < 0) {
        SYSLED_ERR("get sysled loglevel failed, ret: %d\n", ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t sysled_loglevel_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    int ret;

    check_p(g_sysled_drv);
    check_p(g_sysled_drv->set_loglevel);
...
static struct attribute *sysled_dir_attrs[] = {
    &sysled_debug_attr.attr,
    &sysled_loglevel_attr.attr,        
    &sys_led_attr.attr,
    &bmc_led_attr.attr,
    &fan_led_attr.attr,
    &psu_led_attr.attr,
