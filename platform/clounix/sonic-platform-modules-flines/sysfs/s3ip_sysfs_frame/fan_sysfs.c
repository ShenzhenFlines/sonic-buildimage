...
static ssize_t fan_debug_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    int ret;

    check_p(g_fan_drv);
    check_p(g_fan_drv->get_debug);

    ret = g_fan_drv->get_debug(buf, PAGE_SIZE);
    if (ret < 0) {
        FAN_ERR("get fan debug failed, ret: %d\n", ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t fan_debug_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    int ret;

    check_p(g_fan_drv);
    check_p(g_fan_drv->set_debug);
...
static ssize_t fan_loglevel_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    int ret;

    check_p(g_fan_drv);
    check_p(g_fan_drv->get_loglevel);

    ret = g_fan_drv->get_loglevel(buf, PAGE_SIZE);
    if (ret < 0) {
        FAN_ERR("get fan loglevel failed, ret: %d\n", ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t fan_loglevel_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    int ret;

    check_p(g_fan_drv);
    check_p(g_fan_drv->set_loglevel);
                   ...
                   char *buf)
{
    ...
    fan_index = obj->index;
    FAN_DBG("fan index: %u\n", fan_index);
    ret = g_fan_drv->get_fan_vendor_name(fan_index, buf, PAGE_SIZE);
    if (ret < 0) {
...
ssize_t fan_motor_ratio_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int fan_index, motor_index;
    struct switch_obj *p_obj;
    int ret;
    ...
    check_p(g_fan_drv);
    check_p(g_fan_drv->get_fan_motor_ratio);

    p_obj = to_switch_obj(obj->kobj.parent);
    fan_index = p_obj->index;
    motor_index = obj->index;
    FAN_DBG("fan index: %u, motor index: %d\n", fan_index, motor_index);
    ret = g_fan_drv->get_fan_motor_ratio(fan_index, motor_index, buf, PAGE_SIZE);
    if (ret < 0) {
        FAN_ERR("get fan%u motor%u ratio failed, ret: %d\n", fan_index, motor_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}
                   ...
                   const char* buf, size_t count)
{
    ...
    check_p(g_fan_drv);
    check_p(g_fan_drv->set_fan_motor_ratio);
    ...
    if (ret < 0) {
        FAN_ERR("set fan%u motor%u ratio: %d failed, ret: %d\n",
            fan_index, motor_index, ratio, ret);
        return -EIO;
    }
    FAN_DBG("set fan%u, motor%u, ratio: %d success\n", fan_index, motor_index, ratio);
    return count;
}
...
static struct switch_attribute motor_ratio_attr = __ATTR(ratio, S_IRUGO | S_IWUSR, fan_motor_ratio_show, fan_motor_ratio_store);
...
/* create motor[0-(n-1)] directory and attributes in fan directory */
