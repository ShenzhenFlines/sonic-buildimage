...
static ssize_t watchdog_debug_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    int ret;

    check_p(g_wdt_drv);
    check_p(g_wdt_drv->get_debug);

    ret = g_wdt_drv->get_debug(buf, PAGE_SIZE);
    if (ret < 0) {
        WDT_ERR("get watchdog debug failed, ret: %d\n", ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t watchdog_debug_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    int ret;

    check_p(g_wdt_drv);
    check_p(g_wdt_drv->set_debug);
...
static ssize_t watchdog_loglevel_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    int ret;

    check_p(g_wdt_drv);
    check_p(g_wdt_drv->get_loglevel);

    ret = g_wdt_drv->get_loglevel(buf, PAGE_SIZE);
    if (ret < 0) {
        WDT_ERR("get watchdog loglevel failed, ret: %d\n", ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t watchdog_loglevel_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    int ret;

    check_p(g_wdt_drv);
    check_p(g_wdt_drv->set_loglevel);
...
static ssize_t watchdog_state_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    int ret;

    check_p(g_wdt_drv);
    check_p(g_wdt_drv->get_watchdog_state);

    ret = g_wdt_drv->get_watchdog_state(buf, PAGE_SIZE);
    if (ret < 0) {
        WDT_ERR("get watchdog state failed, ret: %d\n", ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    WDT_DBG("get watchdog state success\n");
    return ret;
}
...
static struct switch_attribute watchdog_loglevel_attr = __ATTR(loglevel, S_IRUGO | S_IWUSR, watchdog_loglevel_show, watchdog_loglevel_store);
static struct switch_attribute watchdog_identify_attr = __ATTR(identify, S_IRUGO, watchdog_identify_show, NULL);
static struct switch_attribute watchdog_state_attr = __ATTR(state, S_IRUGO, watchdog_state_show, NULL);
...
static struct attribute *watchdog_dir_attrs[] = {
    &watchdog_debug_attr.attr,
    &watchdog_loglevel_attr.attr,        
    &watchdog_identify_attr.attr,
    &watchdog_state_attr.attr,
